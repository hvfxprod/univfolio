export async function readEditorImage(file: File, maxDimension = 1600): Promise<string> {
...
export async function readProfileImage(file: File): Promise<string> {
  const result = await readEditorImage(file, 512);
  if (result.length > 1000000) throw new Error('프로필 이미지가 너무 큽니다. 다른 이미지를 선택해 주세요.');
  return result;
}
